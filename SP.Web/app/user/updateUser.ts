...
export default angular
        .module('app')
        .controller(controllerId, updateUser).name;

    (updateUser as any).$inject = ['datacontext', '$scope', 'userDetails.abstract', 'tokenStorageService', '$routeParams', 'USER_ROLES'];
        ...
        function submit() {
            ...
            if (vm.permissions.adminLevel !== originalRoles.adminLevel) {
                if (originalRoles.adminLevel) {
                    let rId =  USER_ROLES[originalRoles.adminLevel];
                    vm.participant.roles.find(function (ur) { return ur.roleId === rId; }).entityAspect.setDeleted();
                }
