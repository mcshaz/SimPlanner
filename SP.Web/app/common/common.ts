...
(function () {
    ...
    commonModule.factory('common',
        ['$q', '$rootScope', '$timeout', 'commonConfig', 'logger', 'collectionManager', 'arrayUtils', '$window', common]);
    ...
    function common($q, $rootScope, $timeout, commonConfig, logger, collectionManager, arrayUtils, $window) {
        ...
        */
        interface sm {size:string, min:number, max:number, lte?:(string)=>boolean}
        function getWindowSizes() {
            var min = 0;
            var sizeMap = new Map<string,sm>(([['xs', 767], ['sm', 991], ['md', 1199], ['lg', Number.MAX_SAFE_INTEGER]] as [string,number][])
                .map(function (kv) {
                    var sizeRange:sm = { size: kv[0], min: min, max: kv[1] };
                    sizeRange.lte = lte.bind(sizeRange); 
                    var returnVar:[string,sm] = [sizeRange.size, sizeRange];
                    min = kv[1] + 1;
                    return returnVar;
                }));
            return sizeMap;
            function lte(sz:string) {
        ...
        function currentBootstrapSize() {
            ...
            while (!$window.matchMedia('(max-width: ' + next.value.max + 'px)').matches) {
                ...
                if (i++ >= service.bootstrapSizes.size) { break; }
            }
            return next.value;
        }
    }
