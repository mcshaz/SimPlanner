...
export default angular
    ...
    (controller as any).$inject = ['controller.abstract', '$routeParams', 'common', 'datacontext', '$aside', 'breeze', '$scope', '$location', '$http', '$q', 'loginFactory', 'tokenStorageService'];
    ...
    function controller(abstractController, $routeParams, common, datacontext,  $aside, breeze, $scope, $location, $http, $q, loginFactory, tokenStorageService) {
        ...
        function deleteCourseParticipant(participantId) {
            ...
            datacontext.save(cp).then(function () { vm.log('removed ' + name + ' from course'); },
                function () { vm.log.error({ msg: 'failed to remove ' + name + ' from course' }); });
        }
