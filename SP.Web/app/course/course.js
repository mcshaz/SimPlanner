...
controller.$inject = ['controller.abstract', '$routeParams', 'common', 'datacontext', '$aside', 'breeze', '$scope', '$location', '$http', '$q', 'loginFactory', 'tokenStorageService'];
function controller(abstractController, $routeParams, common, datacontext, $aside, breeze, $scope, $location, $http, $q, loginFactory, tokenStorageService) {
    var vm = this;
    ...
    function dateChanged(dateInst, propName, courseDay) {
        ...
        if (vm.course.courseFormat && dateInst.getHours() === 0 && dateInst.getMinutes() === 0) {
    ...
    function deleteCourseParticipant(participantId) {
        ...
        datacontext.save(cp).then(function () { vm.log('removed ' + name + ' from course'); }, function () { vm.log.error({ msg: 'failed to remove ' + name + ' from course' }); });
    }
    ...
    function adjustCourseDays() {
        getCourseLengthPromise().then(function (courseLength) {
            ...
            vm.courseDays.forEach(function (cd) {
                angular_1.default.extend(cd, courseLength[cd.day]);
            });
        });
