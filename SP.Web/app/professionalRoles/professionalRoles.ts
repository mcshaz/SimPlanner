...
export default angular
    ...
    (controller as any).$inject = ['controller.abstract', '$routeParams', 'common', 'datacontext', '$scope'];
    ...
    function controller(abstractController, $routeParams, common, datacontext, $scope) {
        ...
        function mapRoles(allRoles, departmentRoles) {
            var dict = {};
            var categoryNames = breezeEnums.professionalCategory;
            var keyName;
            vm.categories = [];
                ...
                if (!oldVals.some(function (o) {
                    ...
                    if (pri) {
                        if (pri.entityAspect.entityState.isDeleted()) {
                            ...
                            vm.log.debug({ msg: 'professionalRoleInstitution found in cache & to be added but in state other than deleted', data: pri });
                        }
                    } else {
