...
function service($rootScope, entityManagerFactory, repository, common, config, $q, breeze) {
    ...
    self.save = function () {
        ...
        return self.provider.saveChanges(entititiesToSave)
            ...
            saveResult.entities.forEach(function (el) {
                if (el.entityAspect.instantiationValues) {
                    delete el.entityAspect.instantiationValues;
                }
            });
    ...
    self.courseParticipants = repository.create(self.provider, 'CourseParticipantDto', 'CourseParticipants');
    self.courseScenarioFacultyRoles = repository.create(self.provider, 'CourseScenarioFacultyRoleDto', 'CourseScenarioFacultyRoles');
    self.courseSlots = repository.create(self.provider, 'CourseSlotDto', 'CourseSlots');
    self.courseSlotActivities = repository.create(self.provider, 'CourseSlotActivityDto', 'CourseSlotActivities');
    self.courseSlotManikins = repository.create(self.provider, 'CourseSlotManikinDto', 'CourseSlotManikins');
    self.courseSlotPresenters = repository.create(self.provider, 'CourseSlotPresenterDto', 'CourseSlotPresenters');
    ...
    function cloneItem(item, collectionNames) {
        ...
        function copyChildren(navPropName) {
            ...
            if (navProp.isScalar)
                return;
            var fk = navProp.invForeignKeyNames[0];
            if (!fk)
                return;
