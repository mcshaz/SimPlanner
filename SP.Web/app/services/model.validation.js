...
angular_1.default.module('app').factory(serviceId, ['common', 'breeze', modelValidation]);
function modelValidation(common, breeze) {
    var entityNames;
    var log = common.logger.getLogFn(serviceId);
    var Validator = breeze.Validator, requireReferenceValidator, twitterValidator;
    var service = {
    ...
    function applyRequireReferenceValidators(metadataStore) {
        var navigations = ['aspNetUser', 'country', 'department', 'sessionRoleType', 'scenarioRoleType', 'hospital', 'instructorCourse', 'instructorCourseParticipant', 'manequin', 'professionalRole', 'scenario', 'scenarioResource', 'session', 'sessionParticipant', 'sessionResource', 'sessionType'];
    ...
    function createTwitterValidator() {
        var val = Validator.makeRegExpValidator('twitter', /^@([a-zA-Z]+)([a-zA-Z0-9_]+)$/, "Invalid Twitter User Name: '%value%'");
        return val;
    }
